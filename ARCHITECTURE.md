...
| `DictationController` | State machine tying it all together | — |
...
| While-held pre-transcription | — | hides most of the above | Chunked re-transcription every ~2 s (§4); on release only the tail is new |
| VAD trim | — | saves proportional time | Trim leading/trailing silence (Silero VAD via FluidAudio, or whisper.cpp's built-in) before final pass |
